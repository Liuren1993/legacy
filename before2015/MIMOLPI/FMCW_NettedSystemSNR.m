classdef FMCW_NettedSystemSNR
    % FMCW_NettedSystemSNR calculates the countour graph of the netted FMCW System's SNR
    %   it contains:
    ...
    %     11.the size of NettedReceiverArray is [1,N]
    %     12.the size of NettedTransmitterArray is [1,N]
    %   and calls:
...
%%
    methods
        ...
        function RadiatedPowerSum = get.RadiatedPowerSum(obj)
            ...
            TrArray(1) = []; % remove first unused element
            ...
            for k = 1 : TransmitterNumber   
                TransmitterPositionX = TrArray(1,k).FMCWRadarArray.TransmitterPositionX;
                TransmitterPositionY = TrArray(1,k).FMCWRadarArray.TransmitterPositionY;
                ERP = TrArray(1,k).FMCWRadarArray.ERP;
                for m = 1 : resolution
        ...
        function SNR_Pond = get.SNR_Pond(obj)
            ...
            ReArray(1) = []; % remove first unused element
...
%               CompressRatio = ReArray(1,k).FMCWRadarArray.CompressRatio;
                ReceiverPositionX = ReArray(1,k).FMCWRadarArray.ReceiverPositionX;
                ReceiverPositionY = ReArray(1,k).FMCWRadarArray.ReceiverPositionY;
                Gain_Receiver = ReArray(1,k).FMCWRadarArray.Gain_Receiver;         % None
                BandWidth = ReArray(1,k).FMCWRadarArray.BandWidth;                 % Hz
                NoiseTemperature = ReArray(1,k).FMCWRadarArray.NoiseTemperature;   % K
                NoiseFigure = ReArray(1,k).FMCWRadarArray.NoiseFigure;             % None
                Lambda = ReArray(1,k).FMCWRadarArray.Lambda;                       % m
                RCS = ReArray(1,k).FMCWRadarArray.RCS;                             % m^2
                Loss = ReArray(1,k).FMCWRadarArray.Loss;                           % None
                ...
                for m = 1 : resolution
                    for n = 1 : resolution    % go though everypoint
                        ...
                        ExtremenValue = ( XPointInterval + YPointInterval)/4;
                        range = max( range ,  ExtremenValue );
                        % Received power of point(m,n) by the receiver  CompressRatio*
        ...
        function obj = set.NettedTransmitterArray(obj,trradararray)
            obj.NettedTransmitterArray = trradararray;
            
        end
        function obj = set.NettedReceiverArray(obj,reradararray)
            obj.NettedReceiverArray = reradararray;
            
        end
        %*************** Constructor **********************************
        function obj = FMCW_NettedSystemSNR(resolution,Xboundarystart,Xboundaryend,...
                Yboundarystart,Yboundaryend,trradararray,reradararray)
            if nargin == 0
