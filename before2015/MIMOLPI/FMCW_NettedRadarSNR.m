classdef FMCW_NettedRadarSNR
    % FMCW_NettedRadarSNR calculates the countour graph of the netted FMCW Radar's SNR
    %   it contains:
    ...
    %     11.the size of FMCWRadarArray is [1,N]
    %   and calls:
...
%%
    methods
        ...
        function RadiatedPowerSum = get.RadiatedPowerSum(obj)
            FMCWArray = obj.FMCWRadarArray;
            TransmitterNumber = size(FMCWArray,2);
            ...
            for k = 1 : TransmitterNumber
                TransmitterPositionX = FMCWArray(1,k).FMCWRadarArray.TransmitterPositionX;
                TransmitterPositionY = FMCWArray(1,k).FMCWRadarArray.TransmitterPositionY;
                ERP = FMCWArray(1,k).FMCWRadarArray.ERP;
                for m = 1 : resolution
        ...
        function SNR_Pond = get.SNR_Pond(obj)
            FMCWArray = obj.FMCWRadarArray;
            ReceiverNumber = size(FMCWArray,2);
            XboundaryS = obj.XboundaryStart;
            YboundaryS = obj.YboundaryStart;
            resolution = obj.Resolution;
            XPointInterval = obj.XPointInterval;
            ...
            for k = 1 : ReceiverNumber
                CompressRatio = FMCWArray(1,k).FMCWRadarArray.CompressRatio;
                ReceiverPositionX = FMCWArray(1,k).FMCWRadarArray.ReceiverPositionX;
                ReceiverPositionY = FMCWArray(1,k).FMCWRadarArray.ReceiverPositionY;
                Gain_Receiver = FMCWArray(1,k).FMCWRadarArray.Gain_Receiver;         % None
                BandWidth = FMCWArray(1,k).FMCWRadarArray.BandWidth;                 % Hz
                NoiseTemperature = FMCWArray(1,k).FMCWRadarArray.NoiseTemperature;   % K
                NoiseFigure = FMCWArray(1,k).FMCWRadarArray.NoiseFigure;             % None
                Lambda = FMCWArray(1,k).FMCWRadarArray.Lambda;                       % m
                RCS = FMCWArray(1,k).FMCWRadarArray.RCS;                             % m^2
                Loss = FMCWArray(1,k).FMCWRadarArray.Loss;                           % None
                ...
                for m = 1 : resolution
                    for n = 1 : resolution    % go though everypoint
                        ...
                        parameters = (CompressRatio*Gain_Receiver*RCS*(Lambda)^2) /...
        ...
        function obj = set.FMCWRadarArray(obj,fmcwradararray)
            obj.FMCWRadarArray = fmcwradararray;
            
        end
        %*************** Constructor **********************************
        function obj = FMCW_NettedRadarSNR(resolution,Xboundarystart,Xboundaryend,...
                Yboundarystart,Yboundaryend,fmcwradararray)
            if nargin == 0
