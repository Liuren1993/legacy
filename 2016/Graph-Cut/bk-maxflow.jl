...
function tree_capacity{T<:Number}(p::Node, q::Node, capacity_matrix::AbstractArray{T,2}, flow_matrix::AbstractArray{T,2})
    if TREE(p) == :S
        # @show a = capacity_matrix[p.id,q.id]
        # @show b = flow_matrix[p.id,q.id]
        # return a - b
        return capacity_matrix[p.id,q.id] - flow_matrix[p.id,q.id]
    elseif TREE(p) == :T
        # @show a = capacity_matrix[q.id,p.id]
        # @show b = flow_matrix[q.id,p.id]
        # return a - b
...
function augmentationstage!{T<:Number}(
    ...
    for i = 1:lenPath-1
        p = P[i]
        q = P[i+1]
        # @show capacity_matrix[p.id,q.id]
        # @show flow_matrix[p.id,q.id]
...
function adoptionstage!{T<:Number}(
    ...
    while !isempty(Orphans)
        ...
        for q in neighbors(nodes_vector, p)
            # @show "parentcandidate:", q
            # @show TREE(q)==TREE(p)
            if TREE(q)==TREE(p)
                # @show tree_capacity(q, p, capacity_matrix, flow_matrix)>0
                # @show q.tree != :free
            end
        ...
        if no_valid_parent_flag
            ...
            # @show "deal with no-parent-orphan"
            for q in neighbors(nodes_vector, p)
                # @show q
                ...
                if TREE(q)==TREE(p) && PARENT(q).id == p.id
                    push!(Orphans, q)
                    q.parent = Node()
                end
                # @show Orphans
