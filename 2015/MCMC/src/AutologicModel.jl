...
function pairwise{T}(pair::Tuple{T,T}, beta::Real, label::Real)
    ...
    if isnan(pair[2])
        ...
        pairwisePotential = beta*label*pair[2]
    end
    return pairwisePotential
end

function autologic!{L<:Real,I<:Real,B<:Real}(labelfield::Vector{L}, img::Matrix{I}, alpha::Real, beta::Vector{B})
...
# exports
export pairwise, autologic!, clique
